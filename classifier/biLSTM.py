'''
BiLSTM system for detecting abusive language in multiple data sets
original by Zhang et al: https://bitbucket.org/grunn2018/sharedhate_repo
'''
...
def training_biLSTM(Xtrain, Ytrain, Xtest, Ytest, word_embeds, word_embeds2, tknzr, modelh5, eps, ptc, vb, bs, cv, c):
    if cv:
    ...
    if not word_embeds2:
        print('Using one embedding only..')
        ...
        l_vector = len(word_embeds['a']) * 2                #Stacking embeddings
        embedding_matrix = np.zeros((vocab_size, l_vector)) #Dimension vector in embeddings
        for word, i in t.word_index.items():
            embedding_vector1 = word_embeds.get(word)
            if embedding_vector1 is None:
                embedding_vector1 = [0] * l_vector
            embedding_vector2 = word_embeds2.get(word)
            if embedding_vector2 is None:
                embedding_vector2 = [0] * l_vector
            vec3 = np.hstack((embedding_vector1, embedding_vector2))
            if vec3 is not None and len(vec3) == l_vector:
                embedding_matrix[i] = vec3
...
def output_biLSTM(Xtrain, Ytrain, Xtest, Ytest, tknzr, modelh5, cv, c):
    ...
    print('Loading tokenizer: {}...'.format(tknzr))
    with open(tknzr, 'rb') as handle:
        t = pickle.load(handle)

    print('Tokenizer loaded! Loading model: {}...'.format(modelh5))
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                maxlen_list = [851, 2337, 580, 392, 214, 314, 803]
                for max_length in maxlen_list:
                    try:
                        datalist_reshaped = pad_sequences(datalist_reshaped, maxlen=max_length, padding='post')
                        score = model.predict(datalist_reshaped)
                    except Exception:
    ...
    print('Maxlength: {}, data processed! Predicting values...'.format(max_length))
...
def biLSTM(Xtrain, Ytrain, Xtest, Ytest, training, output, word_embeds, word_embeds2, tknzr, modelh5, cv, eps, ptc, ds, vb, bs, prob):
    if training:
        training_biLSTM(Xtrain, Ytrain, Xtest, Ytest, word_embeds, word_embeds2, tknzr, modelh5, eps, ptc, vb, bs, cv, 0)
        print('Done training')
    ...
    if cv:
        ...
        for train, test in kfold.split(Xtrain, Ytrain):
            ...
            training_biLSTM(X_train.tolist(), Y_train.tolist(), X_test.tolist(), Y_test.tolist(), word_embeds, word_embeds2, tknzr, modelh5, eps, ptc, vb, bs, cv, c)
    ...
    if output:
        print(type(Xtrain), len(Xtrain), Xtrain[:3])
        print(type(Ytrain), len(Ytrain), Ytrain[:3])
        print(type(Xtest), len(Xtest), Xtest[:3])
        print(type(Ytest), len(Ytest), Ytest[:3])
